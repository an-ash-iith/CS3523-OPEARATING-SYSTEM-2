...
//     passenger_params* passenger_parameters = new passenger_params[num_pass];
//     car_params* car_parameters = new car_params[num_cars];
...
//     for (int i = 0; i < num_cars; i++) carStatus[i] = -5;
...
//     delete[] passenger_parameters;
//     delete[] car_parameters;
//     delete[] carStatus;
//     delete[] occupiedCar;
...
//         // we let the passenger wander around for a while to create some time between two successive requests.
...
//         // variable to hold the index of the car allotted to the passenger
//         int car_index = -1;
...
//                 // as soon as it's done, the ride shall begin
...
//         // we make sure there is no race condition by using a semaphore to update the shared variable
...
//         // it establishes itself in the carLine, we acquire the semaphore for this
//         sem_wait(&carLineUpdate);
//         carLine.push_back(car_num);
//         sem_post(&carLineUpdate);
        
//         // it also signals emptyCars to let the passenger know it's available
//         sem_post(&emptyCars);

//         // it also updates its semaphore of being occupied, and allows the passenger to get on
//         sem_post(&occupiedCar[car_num]);

//         // car thread busy waits till it's not allotted a passenger 
