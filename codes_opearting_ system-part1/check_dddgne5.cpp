...
int main() {
    ...
    vector<double> time1(nw, 0);
    vector<double> time2(nr, 0);

    arguments* arg_writer = new arguments[nw];
    arguments* arg_reader = new arguments[nr];

    for (int i = 0; i < nw; i++) {
        arg_writer[i].mucs = mucs;
        arg_writer[i].murem = murem;
        arg_writer[i].no_of_times = kw;
        arg_writer[i].time = &time2;
    ...
    for (int i = 0; i < nw; i++) {
        cout << time2[i] << " ";
        averagetime_writer = averagetime_writer + time2[i];
        if (worsttime_writer < time2[i])
            worsttime_writer = time2[i];
    }
    ...
    for (int i = 0; i < nr; i++) {
        cout << time1[i] << " ";
        averagetime_reader = averagetime_reader + time1[i];
        if (worsttime_writer < time1[i])
            worsttime_writer = time1[i];
    }

    cout << endl;
    cout << "average time taken by reader " << averagetime_writer / nr << endl;
    cout << "worst time taken by reader " << worsttime_writer << endl;
